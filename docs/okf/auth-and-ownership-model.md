...
- Treating `PANEL_TOKEN`-unset dev fallback (`isAdmin: true`, `backend/src/middleware/auth.ts:40-42`) as anything but a local-dev convenience; it never fires when `NODE_ENV === "production"` (`backend/src/middleware/auth.ts:36-39`) and must not be relied on as a security boundary elsewhere.
