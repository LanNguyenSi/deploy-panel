...
| Link | Mechanism | Why |
|---|---|---|
| backend ↔ relay (deploy) | Real SSE, parsed by hand | The relay streams `event:`/`data:` frames over a plain fetch; `streamDeploy` (`backend/src/lib/stream-deploy.ts:229-253`) reads the response body's `ReadableStream` and splits on `\n`, matching `event: `/`data: ` prefixes itself — no `EventSource`, because the caller is a Node `fetch`, not a browser. |
