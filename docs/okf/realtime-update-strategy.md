...
timestamp: 2026-08-22T04:53:32Z
sources:
...
| Link | Mechanism | Why |
|---|---|---|
| backend ↔ relay (deploy) | Real SSE, parsed by hand | The relay streams `event:`/`data:` frames over a plain fetch; `streamDeploy` (`backend/src/lib/stream-deploy.ts:196-220`) reads the response body's `ReadableStream` and splits on `\n`, matching `event: `/`data: ` prefixes itself — no `EventSource`, because the caller is a Node `fetch`, not a browser. |
