...
timestamp: 2026-08-22T07:30:28Z
sources:
...
`streamDeploy` (stream-deploy.ts:100-274) hands `finalizeDeploy` exactly three shapes of relay outcome that could be a success, all converging on the same function:

1. **SSE `done` event** (`handleEvent`, stream-deploy.ts:297-309) — the relay finished streaming and reported `success`/`failure` explicitly.
2. **JSON fallback** (stream-deploy.ts:187-227) — the relay doesn't support streaming and returned a single JSON body instead; `success` is derived from `data.result.success` or `data.deploy.status`.
3. **Stream ended without a `done` event** (stream-deploy.ts:255-268) — the connection closed cleanly but no terminal event arrived; success is inferred from whether every accumulated step reports `"success"` or `"skipped"`.

A fourth shape bypasses `finalizeDeploy` entirely: a **relay 4xx rejection** (stream-deploy.ts:172-181): the relay definitively rejected the deploy request (unknown app, bad branch), not a connection that dropped mid-flight. This is written straight to `status: "failed"` without going through `finalizeDeploy`, because it can never be a `relaySuccess: true` claim needing verification; there is nothing to downgrade. Routing it through the catch block's `recoverBrokenDeploy` instead would run the post-deploy health probe against whatever the PREVIOUS successful deploy left running, greenwashing a real rejection into a "success" row (the bug PR #124 fixed for rollback). 5xx and a missing body still fall through to the generic throw, which the catch block routes through `recoverBrokenDeploy` as before, since those can't be told apart from a dropped connection.
